...
bedtools cluster -i $OUTDIR/input_genome.rfamtr.sorted.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $TRNA_GFF

if [ -e $BASEDIR/input_genome.trna.gff ]
then

cat $BASEDIR/input_genome.trna.gff | sort -k1,1 -k4,4n > $OUTDIR/input_genome.trna.sorted.gff
bedtools cluster -i $OUTDIR/input_genome.trna.sorted.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $OUTDIR/input_genome.trna.nr.gff
...
bedtools cluster -i $OUTDIR/input_genome.alltrna.sorted.gff -d 0 -s | awk -F'\t' '{print $0, ($2~/tRNAscan-SE/)?1:0}' 'OFS=\t' | sort -k10,10 -k11,11nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $TRNA_GFF_2
...
cat $BASEDIR/input_genome.rfam.gff | awk -F'\t' '($3~/SSU_rRNA/||$3~/LSU_rRNA/||$3~/5S_rRNA/)' | grep -v 'rRNA_bacteria' | grep -v 'rRNA_archaea' | sort -k1,1 -k4,4n > $OUTDIR/input_genome.rfamrr.sorted.gff
bedtools cluster -i $OUTDIR/input_genome.rfamrr.sorted.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $RRNA_GFF
...
bedtools cluster -i $OUTDIR/input_genome.rnammer.sorted.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $OUTDIR/input_genome.rnammer.nr.gff
...
bedtools cluster -i $OUTDIR/input_genome.allrrna.sorted.gff -d 0 -s | awk -F'\t' '{print $0, ($2~/RNAmmer/)?1:0}' 'OFS=\t' | sort -k10,10 -k11,11nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $RRNA_GFF_2

fi

OTHER_GFF=$OUTDIR/input_genome.allother.nr.gff

cat $BASEDIR/input_genome.rfam.gff | awk -F'\t' '(!($3~/tRNA/||$3~/SSU_rRNA/||$3~/LSU_rRNA/||$3~/5S_rRNA/))' | grep -v 'rRNA_bacteria' | grep -v 'rRNA_archaea' | sort -k1,1 -k4,4n > $OUTDIR/input_genome.rfam.sorted.gff
...
bedtools cluster -i $OUTDIR/input_genome.allother.sorted.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $OTHER_GFF
...
bedtools cluster -i $OUTDIR/input_genome.final.1.gff -d 0 -s | sort -k10,10 -k6,6nr -u | sort -k1,1 -k4,4n | cut -f 1-9 > $OUTDIR/input_genome.final.2.gff
